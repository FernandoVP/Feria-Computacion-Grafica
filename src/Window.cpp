...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_T) {
		...
		if( theWindow->avanzaCarro <= 33.0f )
			theWindow->avanzaCarro += 0.09f;
		else
			theWindow->avanzaCarro += 0.00f;
	}

	if (key == GLFW_KEY_G) {
		if (theWindow->avanzaCarro >= -18.0f)
			theWindow->avanzaCarro -= 0.09f;
		else
			theWindow->avanzaCarro -= 0.00f;
	}
	...
	if (key == GLFW_KEY_U) {
		if( theWindow->avanzaCarro2 <= 20.0f )
			theWindow->avanzaCarro2 += 0.09f;
		else
			theWindow->avanzaCarro2 += 0.00f;
	}

	if (key == GLFW_KEY_J) {
		if( theWindow->avanzaCarro2 >= -17.0f )
			theWindow->avanzaCarro2 -= 0.09f;
		else
			theWindow->avanzaCarro2 -= 0.00f;
	}
...
{
	Window* theWindow = static_cast<Window*>(glfwGetWindowUserPointer(window));
	glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_HIDDEN); // Esconde mouse
